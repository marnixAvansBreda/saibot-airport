            ...
            var hostBuilder = new HostBuilder()
                ...
                .ConfigureServices((hostContext, services) =>
                {
                    services.AddTransient<IMessageHandler>((svc) =>
                    {
                        ...
                        return new RabbitMQMessageHandler(rabbitMQHost, rabbitMQUserName, rabbitMQPassword, "Pitstop", "FlightScheduleManagement", ""); ;
                    });

                    services.AddTransient<FlightScheduleManagementDBContext>((svc) =>
                    {
                        var sqlConnectionString = hostContext.Configuration.GetConnectionString("FlightScheduleManagementCN");
                        var dbContextOptions = new DbContextOptionsBuilder<FlightScheduleManagementDBContext>()
                            .UseSqlServer(sqlConnectionString)
                            .Options;
                        var dbContext = new FlightScheduleManagementDBContext(dbContextOptions);
